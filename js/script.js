...
function toggleRightSide(open) {
    ...
    const sidepanel = document.getElementById("right_side");
    if (sidepanel) {
        sidepanel.style.right = open ? "0" : "-355px";
    } else {
        console.error("Error: Side panel element not found!");
    }
}
    ...
    const observer = new IntersectionObserver(entries => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          ...
          entry.target.classList.remove('video-in-view');
        }
      });
    }, observerOptions);
        ...
        button.addEventListener('click', () => {
            ...
            accordionButtons.forEach((otherButton, otherIndex) => {
                if (otherButton !== button) {
                    ...
                    otherCollapse.style.maxHeight = null;
                    accordionImages[otherIndex].src = 'Images/icon/plus.png';
            ...
            if (collapse.style.maxHeight) {
                collapse.style.maxHeight = null;
                accordionImages[index].src = 'Images/icon/plus.png';
                accordionImages[index].style.transform = 'rotate(0deg)';
                button.style.backgroundColor = '';
            } else {
                collapse.style.maxHeight = collapse.scrollHeight + "px";
                accordionImages[index].src = 'Images/icon/menus.png';
